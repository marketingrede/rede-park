...
export default class ImportsController {
  ...
  async bulkDestroy({ request, response, session }: HttpContext) {
    ...
    let deletedCount = 0
    for (const id of ids) {
      const record = await SeniorImport.find(id)
      if (record) {
        // 1. Delete associated employees (this also cascades to vehicles!)
        await Employee.query().where('senior_import_id', record.id).delete()

        // 2. Unlink raw upload file
